...
def fill_sleeve(positions, cands, sleeve_id, target_usd, now_iso):
    ...
    for pos in open_pos:
        ...
        if end and datetime.now(UTC) >= end:
            ...
            resolved_count += 1
            log.info(
                "resolved [%s] %s  final_no=%.4f  pnl=$%.2f",
                sleeve_id,
                pos.get("id"),
                cur_no,
                pos["pnl_usd"],
...
def run_once():
    ...
    closed_final = [
        p
        for p in positions
        if isinstance(p, dict) and p.get("worker") == WORKER_NAME and p.get("resolved")
    ]
    ...
    status = {
        ...
        "performance": {
            ...
            "win_rate": (
                sum(1 for p in closed_final if p.get("correct")) / max(1, len(closed_final))
            )
            if closed_final
            else None,
        },
