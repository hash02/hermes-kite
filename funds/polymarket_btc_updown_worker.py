...
def fill_sleeve(positions, cands, sleeve_id, target_usd, now_iso):
    ...
    for pos in open_pos:
        ...
        if end and datetime.now(UTC) >= end:
            pos["resolved"] = True
            ...
            resolved_count += 1
            log.info(
                "resolved [%s] %s  final_no=%.4f  pnl=$%.2f", sleeve_id, pos.get("id"), cur_no, pos["pnl_usd"]
            )
    ...
    for c in cands:
        ...
        opened += 1
        log.info(
            "opened NO [%s] %s yes=%.4f no=%.4f size=$%.2f",
            sleeve_id,
            c["slug"],
            c["yes_price"],
            c["no_price"],
...
def run_once():
    ...
    closed_final = [
        p for p in positions if isinstance(p, dict) and p.get("worker") == WORKER_NAME and p.get("resolved")
    ]
    ...
    status = {
        ...
        "performance": {
            ...
            "win_rate": (sum(1 for p in closed_final if p.get("correct")) / max(1, len(closed_final)))
            if closed_final
            else None,
        },
        "risk": {
    ...
    print(
        f"[{WORKER_NAME}] sleeves={len(per_sleeve_open)} open={len(open_final)} "
