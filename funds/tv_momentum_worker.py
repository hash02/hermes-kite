...
def run_once():
    ...
    # count existing open
    open_self = [p for p in positions if isinstance(p, dict)
    ...
    # status
    open_tv = [p for p in positions if isinstance(p, dict)
               and p.get('worker') == WORKER_NAME and not p.get('resolved')]
    ...
    log.info('cycle=%d opened=%d resolved=%d stopped=%d marked=%d deployed=$%.2f',
             cycle, opened, resolved, stopped, marked, deployed)
    return status
