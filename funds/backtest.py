...
def _size_for_day(leg: SleeveLeg, cum_fund_pnl: float, policy_snapshot: dict, cp_exposure: dict) -> float:
...
def _simulate_one(policy_snapshot: dict, legs: list[SleeveLeg], days: int, rng: random.Random) -> dict:
    ...
    fund_capitals = {
        fid: float(f.get("capital_usd", 1000.0)) for fid, f in policy_snapshot.get("funds", {}).items()
    }
...
def run_mc(policy_snapshot: dict, days: int, sims: int, seed: int, fund_filter: str | None = None) -> dict:
...
def _print_summary(result: dict, label: str = ""):
    ...
    for fid, metrics in result["aggregated"].items():
        ...
        for p in ("p5", "p50", "p95"):
            row = "  ".join(f"{metrics[k][p]:>10.3f}" if k in metrics else "       n/a" for k in keys)
            print(f"  {p:<6}  {row}")
...
def _run_kelly_sweep(
    policy_snapshot: dict, days: int, sims: int, seed: int, fund_filter: str | None, fractions: list[float]
) -> list[dict]:
    rows = []
    for k in fractions:
...
def main():
    ...
    ap.add_argument("--kelly-sweep", action="store_true", help="Sweep kelly_fraction 0.10..1.00 (engine ON)")
    ap.add_argument(
        "--output-dir", type=Path, default=None, help="Write JSON + equity CSVs to this dir (default: skip)"
    )
    args = ap.parse_args()
