...
def run_once():
    ...
    for u in UNIVERSE:
        ...
        if existing:
            ...
            existing['pnl_usd'] = round((mark - entry) / entry * existing.get('size_usd', PRINCIPAL_USD), 4)
    ...
    open_mine = [p for p in positions if isinstance(p, dict)
    ...
    log.info('cycle=%d opened=%s exited=%s stopped=%s held=%s deployed=$%.2f',
             cycle, opened, exited, stopped, held, deployed)
    return status
