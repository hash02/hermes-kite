...
def _accrued_mgmt_fee(gross_equity: float, mgmt_rate_annual: float, days_since_crystallization: int) -> float:
...
def compute_nav(
    fund_id: str, policy: dict | None = None, summary: dict | None = None, ledger: dict | None = None
) -> NavSnapshot | None:
    ...
    perf_accrued = _accrued_perf_fee(nav_net_mgmt, hwm, units, perf_rate, hurdle_rate, days_since_perf)
        ...
        sleeves.append(
            {
                ...
                "pnl_pct_of_capital": round(float(s.get("pnl_usd") or 0) / float(snap.capital_usd) * 100, 4)
                if snap.capital_usd
                else 0,
                "funded": s.get("funded"),
...
def main():
    ...
    ap.add_argument(
        "--crystallize-mgmt", action="store_true", help="Crystallize management fees (moves accrued -> paid)"
    )
    ap.add_argument(
        "--crystallize-perf",
        action="store_true",
    ...
    if args.show or (not args.statement and not args.crystallize_mgmt and not args.crystallize_perf):
        snaps = []
        for fid in fund_ids:
