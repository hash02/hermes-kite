...
def find_pos(positions, symbol):
    for p in positions:
        if isinstance(p, dict) and p.get('worker') == WORKER_NAME and p.get('symbol') == symbol and not p.get('resolved'):
            return p
    return None
...
def run_once():
    ...
    for item in BASKET:
        ...
        if pct_move <= -DOUBLE_DOWN_DROP_PCT:
            ...
            existing['pnl_usd'] = round(existing['size_usd'] - existing['principal_usd'], 4)
            existing.setdefault('grid_ops', []).append({'op': 'double_down', 'price': cur_price, 'add_usd': add_usd, 'time': now_iso})
            double_downs += 1
            actions.append(f"double_down {item['symbol']} +${add_usd:.2f}@${cur_price:.2f}")
            log.info('double_down %s @ $%.2f (avg_entry now $%.2f)', item['symbol'], cur_price, existing['avg_entry_price'])
            ...
            existing.setdefault('grid_ops', []).append({'op': 'trim_half', 'price': cur_price, 'realized': round(realized, 4), 'time': now_iso})
            trims += 1
    ...
    # status
    open_xs = [p for p in positions if isinstance(p, dict) and p.get('worker') == WORKER_NAME and not p.get('resolved')]
    ...
    print(f'[{WORKER_NAME}] open={len(open_xs)} deployed=${deployed:.2f} '
