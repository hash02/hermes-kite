...
def check_book_integrity(settled: dict, report: Report) -> None:
    ...
    gaps = [(a, b) for a, b in zip(sorted_nonces, sorted_nonces[1:]) if b - a != 1]
    if gaps:
        report.add("warn", "book",
                   f"non-contiguous nonce range: gap(s) {gaps[:3]}{'…' if len(gaps) > 3 else ''}")
    # 3. each content_hash is sha256-shaped (64 hex chars)
    bad = [t.get("nonce") for t in txs
           if not _HEX64.match((t.get("content_hash") or "").lower())]
    if bad:
        report.add("error", "book",
                   f"{len(bad)} tx(s) have malformed content_hash (not sha256-64hex)")
    # 4. each tx hash is sha256-shaped too (eth tx hashes are 66 chars inc. 0x, but book strips)
    bad_tx = [t.get("nonce") for t in txs
              if not _HEX64.match((t.get("tx") or "").lower().replace("0x", ""))]
    if bad_tx:
        report.add("error", "book",
                   f"{len(bad_tx)} tx(s) have malformed tx hash")
...
def check_passport_hash(registry: dict, report: Report) -> None:
    ...
    if stored_hash and stored_hash == recomputed:
        report.add("ok", "passport",
                   f"agent passport hash matches (agent_id={registry.get('agent_id')})")
    elif not stored_hash:
        report.add("warn", "passport", "registry has no stored payload_hash")
    else:
        report.add("error", "passport",
...
def check_hashes_vs_txs(settled: dict, report: Report) -> None:
    ...
    if orphan_sleeves:
        ...
        report.add("ok", "book",
                   f"all {len(hashes)} sleeve content hashes have a matching on-chain tx")
...
def check_onchain(settled: dict, wallet: str, rpc_url: str, report: Report) -> None:
    ...
    if nonce_on == max_book_nonce + 1:
        report.add("ok", "nonce",
                   f"on-chain nonce {nonce_on} == book max_nonce+1 ({max_book_nonce}+1)")
    elif nonce_on > max_book_nonce + 1:
        missing = nonce_on - max_book_nonce - 1
        report.add("error", "nonce",
    ...
    for t in sorted(txs, key=lambda x: -(x.get("nonce") or 0))[:20]:
        ...
        try:
            ...
            report.add("error", "tx",
                       f"tx {tx_hash[:12]}… (nonce {t.get('nonce')}) not found on Kite: {e}")
            continue
        if (on_tx.get("from") or "").lower() != wallet.lower():
            report.add("error", "tx",
                       f"tx {tx_hash[:12]}… not from expected wallet")
        else:
            # Quietly passing — don't spam findings
            pass
    # Emit one consolidated ok if nothing flagged
    if not any(f.category == "tx" and f.severity == "error"
               for f in report.findings):
        sampled = min(20, len(txs))
        report.add("ok", "tx",
                   f"sampled {sampled} latest tx(s) — all resolve on Kite and are from wallet")
...
def _print_human(report: Report) -> None:
    ...
    print()
    if report.clean:
        print(f"RESULT: clean ({report.error_count} errors, "
