...
from scripts import reconcile  # noqa: E402
...
class TestBookIntegrity(unittest.TestCase):
    def test_clean_book_passes(self):
        report = reconcile.Report(
            timestamp="", wallet="", rpc="", onchain_checked=False, book_tx_count=0
        )
...
class TestOnchain(unittest.TestCase):
    ...
    def test_nonce_aligned_passes(self):
        ...
        report = reconcile.Report(
            timestamp="", wallet=wallet, rpc="", onchain_checked=False, book_tx_count=3
        )
        ...
        self.assertTrue(any(f.category == "nonce" and f.severity == "ok" for f in report.findings))
    ...
    def test_tx_from_wrong_wallet_detected(self):
        ...
        report = reconcile.Report(
            timestamp="", wallet=wallet, rpc="", onchain_checked=False, book_tx_count=1
        )
        with self._patch_web3(w3):
            reconcile.check_onchain(settled, wallet, "http://x", report)
        errors = [f for f in report.findings if f.severity == "error"]
    ...
    def test_rpc_unreachable_warns_not_errors(self):
        ...
        report = reconcile.Report(
            timestamp="", wallet="0x", rpc="", onchain_checked=False, book_tx_count=0
        )
        with self._patch_web3(w3):
...
class TestReportExit(unittest.TestCase):
    def test_clean_report_reports_clean(self):
        r = reconcile.Report(timestamp="", wallet="", rpc="", onchain_checked=True, book_tx_count=1)
        r.add("ok", "book", "fine")
